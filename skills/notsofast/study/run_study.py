...
def evaluate(pop, hard_fn, rework=REWORK_HARD_HIGH):
    ...
    return {
        ...
        # Rounded so the JSON is byte-stable across interpreters (float summation
        # noise at the 1e-15 level otherwise churns the committed file every re-run).
        "acc_hard_high": round(sum(hh_accs) / len(hh_accs), 6) if hh_accs else None,
